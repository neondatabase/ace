...
var PgsqlHighlightRules = function() {
    ...
    var keywords = (
        ...
        "key|keys|key_member|key_type|label|lag|language|large|last|last_value|lateral|lead|leading" +
        "leakproof|least|left|length|level|library|like|like_regex|limit|link|listagg|listen|ln|load|local" +
        "localtime|localtimestamp|location|locator|lock|locked|log|log10|logged|lower|m|map|mapping|match" +
        "matched|matches|match_number|match_recognize|materialized|max|maxvalue|measures|member|merge" +
        "message_length|message_octet_length|message_text|method|min|minute|minvalue|mod|mode|modifies" +
        "module|month|more|move|multiset|mumps|name|names|namespace|national|natural|nchar|nclob|nested" +
        "nesting|new|next|nfc|nfd|nfkc|nfkd|nil|no|none|normalize|normalized|not|nothing|notify|notnull" +
        "nowait|nth_value|ntile|null|nullable|nullif|nulls|number|numeric|object|occurrences_regex|octets" +
        ...
        "private|privileges|procedural|procedure|procedures|program|prune|ptf|public|publication|quote" +
        ...
        "symmetric|sysid|system|system_time|system_user|t|table|tables|tablesample|tablespace|table_name|tan" +
        "tanh|temp|template|temporary|text|then|through|ties|time|timestamp|timezone_hour|timezone_minute|to" +
        "token|top_level_count|trailing|transaction|transactions_committed|transactions_rolled_back" +
        "transaction_active|transform|transforms|translate|translate_regex|translation|treat|trigger" +
        "trigger_catalog|trigger_name|trigger_schema|trim|trim_array|true|truncate|trusted|type|types" +
        "uescape|unbounded|uncommitted|unconditional|under|unencrypted|union|unique|unknown|unlink|unlisten" +
        "unlogged|unmatched|unnamed|unnest|until|untyped|update|upper|uri|usage|user" +
