...
var PgsqlHighlightRules = function() {
    ...
    var keywords = (
        "a|abort|abs|absent|absolute|access|according|acos|action|ada|add|admin|after|aggregate|all|allocate" +
        "also|alter|always|analyse|analyze|and|any|are|array|array_agg|array_max_cardinality|as|asc" +
        "asensitive|asin|assertion|assignment|asymmetric|at|atan|atomic|attach|attribute|attributes" +
        "authorization|avg|backward|base64|before|begin|begin_frame|begin_partition|bernoulli|between|bigint" +
        "binary|bit|bit_length|blob|blocked|bom|boolean|both|breadth|by|c|cache|call|called|cardinality" +
        "cascade|cascaded|case|cast|catalog|catalog_name|ceil|ceiling|chain|chaining|char|character" +
        "characteristics|characters|character_length|character_set_catalog|character_set_name" +
        "character_set_schema|char_length|check|checkpoint|class|classifier|class_origin|clob|close|cluster" +
        "coalesce|cobol|collate|collation|collation_catalog|collation_name|collation_schema|collect|column" +
        "columns|column_name|command_function|command_function_code|comment|comments|commit|committed" +
        "compression|concurrently|condition|conditional|condition_number|configuration|conflict|connect" +
        "connection|connection_name|constraint|constraints|constraint_catalog|constraint_name" +
        "constraint_schema|constructor|contains|content|continue|control|conversion|convert|copy|corr" +
        "corresponding|cos|cosh|cost|count|covar_pop|covar_samp|create|cross|csv|cube|cume_dist|current" +
        ...
        "datetime_interval_precision|day|db|deallocate|dec|decfloat|decimal|declare|default|defaults" +
        "deferrable|deferred|define|defined|definer|degree|delete|delimiter|delimiters|dense_rank|depends" +
        "depth|deref|derived|desc|describe|descriptor|detach|deterministic|diagnostics|dictionary|disable" +
        ...
        "document|domain|double|drop|dynamic|dynamic_function|dynamic_function_code|each|element|else" +
        "empty|enable|encoding|encrypted|end|end-exec|end_frame|end_partition|enforced|enum|equals|error" +
        "escape|event|every|except|exception|exclude|excluding|exclusive|exec|execute|exists|exp|explain" +
        "expression|extension|external|extract|false|family|fetch|file|filter|final|finalize|finish|first" +
        "first_value|flag|float|floor|following|for|force|foreign|format|fortran|forward|found|frame_row" +
        "free|freeze|from|fs|fulfill|full|function|functions|fusion|g|general|generated|get|global|go|goto" +
        "grant|granted|greatest|group|grouping|groups|handler|having|header|hex|hierarchy|hold|hour|id" +
        "identity|if|ignore|ilike|immediate|immediately|immutable|implementation|implicit|import|in|include" +
        "including|increment|indent|index|indexes|indicator|inherit|inherits|initial|initially|inline|inner" +
        "inout|input|insensitive|insert|instance|instantiable|instead|int|integer|integrity|intersect" +
        "intersection|interval|into|invoker|is|isnull|isolation|join|json_array|json_arrayagg|json_exists" +
        "json_object|json_objectagg|json_query|json_table|json_table_primitive|json_value|k|keep|key|keys" +
        "key_member|key_type|label|lag|language|large|last|last_value|lateral|lead|leading|leakproof|least" +
        "left|length|level|library|like|like_regex|limit|link|listagg|listen|ln|load|local|localtime" +
        "localtimestamp|location|locator|lock|locked|log|log10|logged|lower|m|map|mapping|match|matched" +
        "matches|match_number|match_recognize|materialized|max|maxvalue|measures|member|merge|message_length" +
        "message_octet_length|message_text|method|min|minute|minvalue|mod|mode|modifies|module|month|more" +
        "move|multiset|mumps|name|names|namespace|national|natural|nchar|nclob|nested|nesting|new|next|nfc" +
        "nfd|nfkc|nfkd|nil|no|none|normalize|normalized|not|nothing|notify|notnull|nowait|nth_value|ntile" +
        "null|nullable|nullif|nulls|null_ordering|number|numeric|object|occurrence|occurrences_regex|octets" +
        "octet_length|of|off|offset|oids|old|omit|on|one|only|open|operator|option|options|or|order|ordering" +
        ...
        "percentile_disc|percent_rank|period|permission|permute|pipe|placing|plan|plans|pli|policy|portion" +
        "position|position_regex|power|precedes|preceding|precision|prepare|prepared|preserve|prev|primary" +
        "prior|private|privileges|procedural|procedure|procedures|program|prune|ptf|public|publication|quote" +
        "quotes|range|rank|read|reads|real|reassign|recheck|recovery|recursive|ref|references|referencing" +
        "refresh|regr_avgx|regr_avgy|regr_count|regr_intercept|regr_r2|regr_slope|regr_sxx|regr_sxy|regr_syy" +
        "reindex|relative|release|rename|repeatable|replace|replica|requiring|reset|respect|restart|restore" +
        "restrict|result|return|returned_cardinality|returned_length|returned_octet_length" +
        "returned_sqlstate|returning|returns|revoke|right|role|rollback|rollup|routine|routines" +
        "routine_catalog|routine_name|routine_schema|row|rows|row_count|row_number|rule|running|savepoint" +
        "scalar|scale|schema|schemas|schema_name|scope|scope_catalog|scope_name|scope_schema|scroll|search" +
        "second|section|security|seek|select|selective|self|semantics|sensitive|sequence|sequences" +
        "serializable|server|server_name|session|session_user|set|setof|sets|share|show|similar|simple|sin" +
        "sinh|size|skip|smallint|snapshot|some|sort_direction|source|space|specific|specifictype" +
        "specific_name|sql|sqlcode|sqlerror|sqlexception|sqlstate|sqlwarning|sqrt|stable|standalone|start" +
        "state|statement|static|statistics|stddev_pop|stddev_samp|stdin|stdout|storage|stored|strict|string" +
        "strip|structure|style|subclass_origin|submultiset|subscription|subset|substring|substring_regex" +
        "succeeds|sum|support|symmetric|sysid|system|system_time|system_user|t|table|tables|tablesample" +
        "tablespace|table_name|tan|tanh|temp|template|temporary|text|then|through|ties|time|timestamp" +
        ...
        "trusted|type|types|uescape|unbounded|uncommitted|unconditional|under|unencrypted|union|unique" +
        "unknown|unlink|unlisten|unlogged|unmatched|unnamed|unnest|until|untyped|update|upper|uri|usage|user" +
        ...
        "value_of|varbinary|varchar|variadic|varying|var_pop|var_samp|verbose|version|versioning|view|views" +
        "volatile|when|whenever|where|whitespace|width_bucket|window|with|within|without|work|wrapper|write" +
